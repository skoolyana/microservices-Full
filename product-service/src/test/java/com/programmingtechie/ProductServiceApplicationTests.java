...
class ProductServiceApplicationTests {
	...
	{ 
		dynamicPropertyRegistery.add("spring.data.mongodb.uri", 
	...
	void shouldCreateProduct() throws Exception {
		...
		ProductRequest prod_Request =  getProductRequest();
		
		String productRequestJson = objectMapper.writeValueAsString(prod_Request);
		
		mockMvc.perform(MockMvcRequestBuilders.post("/api/product")
				.contentType(MediaType.APPLICATION_JSON)
				.content(productRequestJson)).andExpect
		(status().isCreated());
	...
	private ProductRequest getProductRequest() {
		...
		return ProductRequest.builder().name("iPhone13").description("Sunil Phone")
